  ...
  function updateFreeShippingBar(totalPrice) {
    ...
    if (message) {
      if (remaining === 0) {
        ...
        message.innerHTML = `<span class="tw-text-velox-700">${(bar.dataset.freeShippingRemaining || '{amount} away from free shipping').replace('{amount}', formattedRemaining)}</span>`;
      }
    }
  }
